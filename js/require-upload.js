define(['jquery', 'bootstrap', 'plupload', 'template'], function ($, undefined, Plupload, Template) {
    var Upload = {
        ...
        config: {
            ...
            /*previewtplvideo: '<li class="col-xs-3"><a href="<%=fullurl%>" data-url="<%=url%>" target="_blank" class="thumbnail"><img src="<%=fullurl%>?x-oss-process=video/snapshot,t_1000,m_fast" onerror="this.src=\'https://tool.fastadmin.net/icon/\'+\'<%=fullurl%>\'.split(\'.\').pop()+\'.png\';this.onerror=null;" class="img-responsive"></a><a href="javascript:;" class="btn btn-danger btn-xs btn-trash"><i class="fa fa-trash"></i></a></li>',*/
            previewtplvideo: '<li class="col-xs-3"><a href="/player/videoDetail.html" data-videourl="<%=fullurl%>"  target="_blank" class="thumbnail"><img src="<%=fullurl%>?x-oss-process=video/snapshot,t_1000,m_fast" onerror="this.src=\'https://tool.fastadmin.net/icon/\'+\'<%=fullurl%>\'.split(\'.\').pop()+\'.png\';this.onerror=null;" class="img-responsive"></a><a href="javascript:;" class="btn btn-danger btn-xs btn-trash"><i class="fa fa-trash"></i></a></li>',
            previewtpl: '<li class="col-xs-3"><a href="<%=fullurl%>" data-url="<%=url%>" target="_blank" class="thumbnail"><img src="<%=fullurl%>!unorient" onerror="this.src=\'https://tool.fastadmin.net/icon/\'+\'<%=fullurl%>\'.split(\'.\').pop()+\'.png\';this.onerror=null;" class="img-responsive"></a><a href="javascript:;" class="btn btn-danger btn-xs btn-trash"><i class="fa fa-trash"></i></a></li>',
        },
        events: {
            //初始化完成
            ...
            onUploadError: function (up, ret) {
                ...
                if( ret.code =='-600' ){
                    Toastr.error("您上传的文件超过200M");
                }
                if( ret.code =='-601' ){
                    Toastr.error("您上传的文件格式不对");
                }
                if( ret.code !='-600' && ret.code !='-601' ){
                    Toastr.error(ret.msg + "(code:" + ret.code + ")");
                }
        ...
        api: {
            ...
            plupload: function (element, onUploadSuccess, onUploadError, onUploadComplete) {
                ...
                $(element, Upload.config.container).each(function () {
                    ...
                    var videotpl = $(this).data("video"); // 判断是否是视频，确认渲染回显的模板
                    ...
                    if (preview_id && input_id) {
                        $(document.body).on("keyup change", "#" + input_id, function () {
                            ...
                            $.each(inputArr, function (i, j) {
                                ...
                                if(videotpl){
                                    var html = tpl ? Template(tpl, data) : Template.render(Upload.config.previewtplvideo, data);
                                }else{
